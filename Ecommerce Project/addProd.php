...
if($_SERVER["REQUEST_METHOD"] =="POST"){
        if(!empty($_POST['ProdName']) || !empty($_POST['ProdDesc'])){
            ...
            $ProdDesc = strip_tags($_POST['ProdDesc']);
            $prodPrice = $_POST['ProdPrice'];
            $belongsToCateg = $_POST['belongsToCateg'];
        if(isset($_POST['btnAddProd'])){
            try{
            $sql_Prod_add = $conn->prepare("INSERT INTO `Products`(`ProdName`, `ProdDesc`, `prodPrice`, `belongsToCateg`) VALUES (:Prodname, :ProdDesc, :prodPrice, :belongsToCateg)");
            $sql_Prod_add->execute([":Prodname"=>$ProdName, ":ProdDesc"=>$ProdDesc, ":prodPrice"=>$prodPrice, ":belongsToCateg"=>$belongsToCateg]);
                            ...
                            <div class="card-body">
                                ...
                                <div class="m-sm-4">
                                    ...
                                    <form method="post">
                                        ...
                                        <div class="mb-3">
                                            <textarea id="prodPrice" name="ProdDesc" style="width:100%;" rows="5" placeholder="Describe your Product"></textarea>
                                        </div>
                                        <div class="mb-3">
                                            <input class="form-control form-control-lg" type="number" name="ProdPrice" placeholder="Price"/>
                                        </div>
                                        <div class="mb-3">
                                            <input class="form-control form-control-lg" type="text" name="belongsToCateg" placeholder="Belongs To Categegory">
                                        </div>
