...
$id = $ProdDesc = $ProdName = $ProdPrice = $belongsToCateg = "";
...
while($rows= $sql_prodEdit->fetch(PDO::FETCH_ASSOC)){
    ...
    $prodDesc = strip_tags($rows['prodDesc']);
    $prodPrice = $rows['prodPrice'];
    $belongsToCateg = $rows['belongsToCateg'];
}
if(isset($_POST['btnEditProd'])){ echo 0;
if(isset($_POST['id'])){ echo 1;
    $stmnt_ProdUpdate = $conn->prepare("UPDATE products SET prodName=:pName, prodDesc=:pDesc, prodPrice=:pPrice, belongsToCateg=:belongsToCateg WHERE id=:id");
    ...
    $stmnt_ProdUpdate->bindParam(":belongsToCateg", $_POST['belongsToCateg']);
                        ...
                        <div class="card-body">
                            <div class="m-sm-4">
                                ...
                                <form method="post">
                                    ...
                                    <div class="mb-3">
                                        <input class="form-control form-control-lg" type="text" name="belongsToCateg" required value=<?php echo $belongsToCateg; ?> >
                                    </div>
                                    <div class="text-center mt-3">
                                        <button type="submit" class="btn btn-lg btn-primary" name=btnEditProd>Save</button>
                                    </div>
                                </form>
                            </div>
                        </div>
